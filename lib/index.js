...
export class UsageBadgeRuntime extends TypertRemoteService {
   ...
   *
   * Caller abort signals are intentionally ignored: the in-flight fetch is
   * shared across callers, so one caller's unmount-abort must not cancel the
   * fetch everyone else is awaiting. The fetch is bounded by its own timeout
   * (`USAGE_TIMEOUT_MS`); a fetch that outlives its caller still fills the
   * cache, which is the desired outcome.
   * @param provider - the model provider id the client reports.
   * @param force - bypass the cache and fetch a fresh report (badge click).
   * @param signal - accepted for the gateway contract; ignored (see above).
